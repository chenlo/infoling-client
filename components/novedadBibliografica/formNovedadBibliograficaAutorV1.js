...
function FormNovedadBibliograficaAutor({ autores, setAutores }) {
  ...
  const [autorIndexSelected, setAutorIndexSelected] = useState(0)

  useEffect(() => {
    if (autorIndexSelected && autorIndexSelected > 0) {
      setNombre(autores[autorIndexSelected - 1].nombre)
      setPrimerApellido(autores[autorIndexSelected - 1].primerApellido)
      setSegundoApellido(autores[autorIndexSelected - 1].segundoApellido)
      setTipo(autores[autorIndexSelected - 1].tipo)
    }
  }, [autorIndexSelected]);
        ...
        { autorIndexSelected === 0 ? (
          ...
          <button
            ...
            onClick={(e) => {
              ...
              if (tipo && nombre && primerApellido) {
                let arrayAutoresAux = [
                  ...autores.slice(0, autorIndexSelected-1),
                  updatedAutor,
                  ...autores.slice(autorIndexSelected-1 + 1)
                ];
                ...
                setSegundoApellido('')
                setTipo('autor')
                setAutorIndexSelected(0);
              }
            }}
          >
            <ArrowPathIcon className="flex-shrink-0 w-6 h-6 mr-2" />Guardar cambios
          </button>
        )}
      </div>
      <FormNovedadesBibliograficasAutoresLista autores={autores} setAutores={setAutores} setAutorIndexSelected={setAutorIndexSelected} />
    </fieldset >
  );
}
