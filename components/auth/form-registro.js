        ...
        <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-start">
                ...
                <div className="ml-3 text-sm">
                    <label htmlFor="terms" className="font-light text-gray-500 dark:text-gray-300">Acepto los <a className="font-medium text-primary-600 hover:underline dark:text-primary-500" href="/terminos-condiciones">Términos y condiciones</a></label>
                </div>
            </div>
            <button type="submit" className="disabled:opacity-25 px-6 py-2 text-white bg-indigo-600 rounded-md md:ml-5" disabled={!terms | !name | !email | !password} >
                {loading ? <ThreeDots height="25" width="40" color="#fff" ariaLabel="three-dots-loading" /> : "Enviar"}
            </button>
            <p className="text-sm font-light text-gray-500 dark:text-gray-400">
                ¿Ya se ha registrado? <a href="/login" className="font-medium text-primary-600 hover:underline dark:text-primary-500">Identifíquese aquí</a>
            </p>
        </form>
    )
}
